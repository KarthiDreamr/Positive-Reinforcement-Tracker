...
function App() {
  ...
  const handleStart = async (
    ...
    // Compute and persist a persistent absolute deadline for the timer
    const now = Date.now();
    const endAt = enableTimer ? now + minutes * 60000 : null;
    console.debug('[Timer] handleStart: setting timerEndAt', { enableTimer, minutes, endAt });

    await storage.setUseTaskNames(enableTaskNames);
    await storage.setUseTimer(enableTimer);
    await storage.setTimerMinutes(minutes);
    await storage.setTimerEndAt(endAt);
  ...
  const handleReset = async () => {
    storage.clearAll();
    console.debug('[Timer] handleReset: cleared all including timerEndAt');
    setTotalTasks(null);
    setReward('');
    setTaskNames([]);
    ...
    <Layout onReset={handleReset} soundEnabled={soundEnabled} onSoundToggle={toggleSound}>
      <div className="w-full max-w-md">
        {totalTasks === null ? (
          ...
          <TaskNameSetup totalTasks={totalTasks} onComplete={handleTaskNamesComplete} onBack={handleBack} />
        ) : (
          <Tracker
